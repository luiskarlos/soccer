...
public class Injector {
	...
	public Injector() {
		...
		instances.put(ScriptParser.class, new SingletonFactory() {
			@Override
			public Object newInstance() {
				return new ScriptParser(get(Enviroment.class));
			}
		});
		...
		instances.put(Enviroment.class, new SingletonFactory() {
			...
			public Object newInstance() {
				...
				Enviroment env = new Enviroment(get(EntityBuilder.class), get(Parameters.class), get(Console.class),
				    dispatcher, pFsms);
				return env;
			}
		});
		...
		get(Enviroment.class).registerStates(states);
	}
	...
	public StateMachine newStateMachine(StateMachineOwner owner, String globalState) {
		final StateMachine stateMachine = new StateMachine(owner, getProvider(Evaluator.class));
